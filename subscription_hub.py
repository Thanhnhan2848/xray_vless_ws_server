...
def merge_node_files(nodes_dir: Path, output_path: Path) -> int:
    seen: set[str] = set()
    merged: list[str] = []
...
def make_handler(token: str, nodes_dir: Path, output_path: Path):
    class SubscriptionHubHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            with WRITE_LOCK:
                ...
                total = merge_node_files(nodes_dir, output_path)
            print(f"[OK] Synced node '{node_id}': {len(links)} links; merged total: {total}")
            self.send_json(HTTPStatus.OK, {"ok": True, "node_id": node_id, "total_links": total})
...
def main() -> None:
    ...
    handler = make_handler(args.token, args.data_dir, args.output)
